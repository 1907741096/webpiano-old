...
<div class="news_info_info">
	<div class="news_info_left">
		<div class="news-time"><?php echo (date('Y-m-d H:i',$news["create_time"])); ?></div>
		<div class="news-author">
			<img src="<?php echo ($news["face"]); ?>">
			<div><?php echo ($news["author"]); ?></div>
		</div>
		<div class="news-title"><?php echo ($news["title"]); ?></div>
		<?php if($news['thumb']!=''): ?><img src="<?php echo ($news["thumb"]); ?>" height="300"><?php endif; ?>
		<div class="news-content"><?php echo ($news["content"]); ?></div>

		<div class="news-comment">评论 <?php echo ($news["comment"]); ?></div>
		<div class="news-comment-content">
			<?php if(is_array($comments)): $i = 0; $__LIST__ = $comments;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$comment): $mod = ($i % 2 );++$i;?><div class="comments-author">
					<img src="<?php echo ($comment["face"]); ?>">
		...
		<div class="comment_form">
			<form id="form">
				...
				<input class="news_form_button" type="button" id="submit" value="评论">
			</form>
		</div>
	</div>
...
<script type="text/javascript">
	var SCOPE={
		'save_url':'index.php?c=comment&a=add',
		'jump_url':'index.php?c=news&a=info&id=<?php echo ($news["news_id"]); ?>'
	};
</script>
</body>
</body>